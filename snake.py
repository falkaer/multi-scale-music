...
def reverse_broadcast_to(x, y):
    dims = (Ellipsis,) + (None,) * (y.ndim - x.ndim)
    return x[dims]
...
if __name__ == '__main__':
    torch.manual_seed(0)
    # S = Snake(128, init=0.5, correction='std').cuda()
    S = Snake(128, init=0.5, correction='std').cuda()
    x = torch.randn(16, 128, 16000, device='cuda', dtype=torch.half, requires_grad=True)
    
    cr = snake_correction(S.alpha, S.correction)
    out1 = (x + 1 / S.alpha[..., None] * torch.sin(S.alpha[..., None] * x) ** 2) / cr[..., None]
    # out1 = x + 1 / S.alpha[..., None] * torch.sin(S.alpha[..., None] * x) ** 2
    y1 = out1.sum()
    y1.retain_grad()
    y1.backward(retain_graph=True)
    
    xgrad1 = x.grad.clone()
    alphagrad1 = S.alpha.grad.clone()
    ...
    xgrad2 = x.grad.clone()
    alphagrad2 = S.alpha.grad.clone()
    
    print(torch.allclose(xgrad1, xgrad2, atol=1e-5))
    print(torch.allclose(alphagrad1, alphagrad2, atol=1e-5))
    ...
    # y1 = (x + 1 / S.alpha[..., None] * torch.sin(S.alpha[..., None] * x) ** 2)  # / cr[..., None]
    # y1.sum().backward()
    # alphagrad1 = S.alpha.grad.clone()
    # xgrad1 = x.grad.clone()
    # 
    ...
    # print(torch.allclose(alphagrad1, alphagrad2))
    # print(torch.allclose(xgrad1, xgrad2, atol=1e-6))
    
    # grad = torch.randn_like(x)
    # out = snake_fwd_triton(x, S.alpha, cr)
    # dydx1, dyda1, dydc1 = snake_bwd_torch(x, S.alpha[..., None], cr[..., None], out, grad, True, True, True)
    # dydx2, dyda2, dydc2 = snake_bwd_triton(x, S.alpha, cr, out, grad, True, True, True)
    # print(out)
    # print(dydx2, dyda2, dydc2)
    # 
    # print(torch.allclose(dydx1, dydx2, atol=1e-5))
    # print(torch.allclose(dyda1, dyda2, atol=1e-5))
    # print(torch.allclose(dydc1, dydc2, atol=1e-5))
    # 
    # print()
